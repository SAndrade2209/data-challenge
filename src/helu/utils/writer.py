...
from pyspark.sql.functions import current_timestamp, lit, to_date, to_utc_timestamp
...
class Writer:
    ...
    def add_metadata_columns(self, df: DataFrame, layer: str = None) -> DataFrame:
        if layer is None:
            return (
                ...
                .withColumn(
                    "_inserted_date_utc",
                    to_date(to_utc_timestamp(current_timestamp(), "UTC")),
                )
            )

        return df.withColumn(
            f"_{layer}_ingestion_timestamp_utc",
            to_utc_timestamp(current_timestamp(), "UTC"),
        ).withColumn(
            "_inserted_date_utc", to_date(to_utc_timestamp(current_timestamp(), "UTC"))
        )
    ...
    def append_data(self, df: DataFrame, custom_table_name: str = None) -> str:
        ...
        logger.info("Finished appending to raw")

    def get_update_schema(self, df: DataFrame):
        columns = df.columns
        should_not_updated_columns = ["_ingestion_timestamp_utc"]
        update_scheme = {
            i: f"s.{i}" for i in columns if i not in should_not_updated_columns
        }  # Deprecated column kitchen_ingestion_timestamp
    ...
    def upsert_data(self, df_new: DataFrame, update_schema=None):
        ...
        if self.job_parameters.update_condition:
            delta_df.alias("t").merge(
                df_new.alias("s"), self.job_parameters.merge_logic
            ).whenMatchedUpdate(  # type: ignore
    ...
    def create_or_upsert(self, df: DataFrame):
        if DeltaTable.isDeltaTable(self.spark, self.final_table_path):
            logger.info(
                f"Delta table already exists at {self.final_table_path}. Starting upsert."
            )
