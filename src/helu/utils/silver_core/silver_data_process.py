...
from pyspark.sql.functions import col, trim, lower, to_timestamp, row_number, lit, split
...
class SilverPipeline:
    def __init__(self, spark: SparkSession, job_parameters: JobParameters, writer: Writer=None):
        ...
        self.destination_table_path = f"{self.destination_path}/{self.job_parameters.source}"
    ...
    def read_bronze(self, filter_date: str = None) -> DataFrame:
        ...
        if DeltaTable.isDeltaTable(self.spark, self.destination_table_path):
            ...
            silver_df = self.spark.read.format("delta").load(self.destination_table_path)
            last_processing_date = PipelineFunctions.get_last_processing_date(silver_df, filter_date)

            if last_processing_date:
                logger.info(f"Last processing date found: {last_processing_date}. Reading new data from bronze.")
                bronze_df = df.where(f"{filter_date} >= '{last_processing_date}'"
                )
                return bronze_df
            else:
                logger.info("No last processing date found. Reading all data from bronze.")
                return df
        else:
    ...
    def remove_invalid_data(self, df: DataFrame) -> DataFrame:
        logger.info(f"Removing invalid data")
        is_valid = (
        ...
        if df_invalid and not df_invalid.isEmpty():
            logger.info(f"Saving invalid data")
            df_invalid = df_invalid.withColumn("_invalid_reason", lit("id_data_is_null"))

            self.writer.append_data(
                df=df_invalid, custom_table_name=f"{self.job_parameters.source}_invalid_data"
            )
    ...
    def cast_timestamps_data(self, df: DataFrame) -> DataFrame:
        ...
        return df.withColumn(
            ...
            "signup_timestamp",
            to_timestamp(col("signup_timestamp"))
        )

    def remove_duplicates(self, df: DataFrame) -> DataFrame:
        logger.info(f"Removing duplicate data")
        df_row = df.withColumn("row_number",
                                          row_number().over(Window.partitionBy(self.job_parameters.identifier).orderBy(col(self.job_parameters.deduplication_sorting_column).desc()))
                                          )
        df_deduplicated = df_row.where("row_number = 1").drop(df_row.row_number)
        logger.info(f"Finished removing duplicates")
    ...
    def move_to_silver(self, filter_date: str = None):
        ...
        if bronze_df and not bronze_df.isEmpty():
            ...
            if self.job_parameters.is_dimension:
                logger.info(f"Moving dimension table")
                ...
                logger.info(f"Moving fact table")
